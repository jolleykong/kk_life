...
- 安装Golang
  ...
  ```
  # 拉取repo
  root@ms51:~ # curl -s https://mirror.go-repo.io/centos/go-repo.repo | tee /etc/yum.repos.d/go-repo.repo
  
  # 安装golang，以及git
  root@ms51:~ # yum install go git -y
  
  # 确保go在PATH中
  root@ms51:~ # go version
...
- 拉取Soar源码并编译
  ...
  ```
  root@ms51:~ # mkdir soar
  root@ms51:~ # export GOPATH=/root/soar
  root@ms51:~ # go get -d github.com/XiaoMi/soar
  root@ms51:~ # cd src/github.com/XiaoMi/soar
  root@ms51:~ # make
  ```
...
- 验证Soar
  ...
  ```
  # 随便写个SQL，不用连接库
  root@ms51:bin git:(dev*) # echo "select * from bb where id = 1" | ./soar
  # Query: E3BD63ED6D8F189A
...
## Soar 简单探索：SQL指纹规则和分数

- SQL指纹相同，因为执行计划相同。--> select ... where id=xxx; --> hash --> SQL指纹

  ```
  [23:05:30] root@ms51:bin git:(dev*) # echo "select * from bb where id = 1" | ./soar
  # Query: E3BD63ED6D8F189A
  
  ...
  ...
  
  [23:05:45] root@ms51:bin git:(dev*) # echo "select * from bb where id = 2" | ./soar
  # Query: E3BD63ED6D8F189A
  ```
...
- SQL评分建议
  ...
  * **Content:**  当表结构变更时，使用 \* 通配符选择所有列将导致查询的含义和行为会发生更改，可能导致查询返回更多的数据。
  ...
  [23:08:40] root@ms51:bin git:(dev*) # echo "select id from bb where id = 2" | ./soar
  # Query: F6074960E4DD29C0
...
- 不同类型的建议制定的Severity不同，严重程度数字由低到高依次排序，满分100分，扣到0分为止。
- L0不扣分，只给出建议
- L1扣5分，L2扣10分，逐级递增5分。
- 当建议重叠时，扣分叠加，如：收到L1、L2两个建议，扣15分。



#### SQL评分规则实现

- advisor/rules.go                           # 增加一条新规则
...
- soar.yaml
- `echo "select ..." | ./soar -config ./soar.yaml`
- `./soar -config ./soar.yaml -query " select ..."`
- `./soar -config ./soar.yaml -query ./query.sql`

### Soar 通过命令行连接数据库

```
[23:44:51] root@ms51:etc git:(dev*) # echo " select * from world.city where name like 'A%' "| ../bin/soar -test-dsn="root:root@127.0.0.1:3306/world" -allow-online-as-test     
# Query: 5A00DF732C81ABCD
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1  | SIMPLE | *city* | NULL | ALL | NULL | NULL | NULL | NULL | 4046 | 11.11% | ☠️ **O(n)** | Using where |
...
* **Case:** ALTER TABLE \`world\`.\`city\` add index \`idx\_name\` (\`name\`) ;
...
[23:47:39] root@ms51:etc git:(dev*) # 
...
- 编辑配置文件
  ...
  ```
  [0:05:51] root@ms51:bin git:(dev*) # cat  ../etc/soar.yaml                                                                
  # 这是一个配置文件例子
  online-dsn:
      addr:     127.0.0.1:3306
      schema:   sakila
...
- Soar通过配置文件调用数据库
  ...
  ```
  [0:06:34] root@ms51:bin git:(dev*) # echo " select * from world.city where name like 'A%' "| ./soar -config ../etc/soar.yaml
  # Query: 5A00DF732C81ABCD
...
1. 检查SQL指纹，如果SQL指纹不存在于指纹库，且分数较低，说明这是一个需要关注的新SQL
2. 使用soar配合慢日志处理

### 案例：使用soar配合慢日志处理

- 初始慢日志配置`long_query_time=10`，观察一下每天慢日志有多少。使用soar处理这些慢SQL。
- 当初始配置慢日志变少或消失时，调整配置`long_query_time=1`，观察一下每天慢日志有多少。使用soar处理这些慢SQL。
- 当1s配置慢日志变少或消失时，调整配置`long_query_time=0.5`，观察一下每天慢日志有多少。使用soar处理这些慢SQL。
- 最终达到消除特定时间段慢日志，甚至消除全部慢日志。



- 通过ELK，可以计算出每个机器每天有多少条慢SQL，每个SQL有多少条，高峰期慢SQL都有哪些。

![ ](.pics/image-20201022100836998.png)

- 高级一点的架构

![](.pics/image-20201022101233972.png)
...
- Soar参数
  ...
  ```
  [23:24:22] root@ms51:soar git:(dev*) # bin/soar -h
  Usage of bin/soar:
